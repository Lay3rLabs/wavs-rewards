// This script loads environment variables from rewards_deploy.json and the root .env file
...
const rewardsDeployPath = path.resolve(__dirname, '../.docker/rewards_deploy.json');
...
function loadAndTransformEnv() {
  try {
    // Check if rewards_deploy.json exists
    if (!fs.existsSync(rewardsDeployPath)) {
      console.error('ERROR: No rewards_deploy.json file found at', rewardsDeployPath);
    ...
    // Parse the rewards_deploy.json file
    const deployConfig = JSON.parse(fs.readFileSync(rewardsDeployPath, 'utf8'));
    ...
    let fileContent = '# This file is auto-generated from rewards_deploy.json and root .env\n';
